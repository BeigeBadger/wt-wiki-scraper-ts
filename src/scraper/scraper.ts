...
export async function scrapeCategory(
  ...
  try {
    ...
    for (const mode of ['arcade', 'realistic', 'simulator'] as BattleRatingMode[]) {
      ...
      if (mode !== 'arcade') {
        ...
        try {
          ...
          logError('scraping', 'all', category, page.url(), `Failed to switch to ${mode}: ${error}`);
          continue;
        }
      }
    ...
    logError('scraping', 'all', category, page.url() || `${BASE_URL}/${category}`, `Unexpected error: ${error}`);
  } finally {
    await page.close();
  }
}
